...
func _ready():
	...
	Global.llama.set_sys_prompt("You are an AI Large Language Model trained to generate a random Stable Diffusion prompt for Dreamshaper XL. You never include style in your prompt. You never refer to artists, copyrighted works, copyright owners, or anything else that may infringe copyright in your prompt.")
...
func _deferred_llama_finish():
	if Global.llama != null && Global.llama.is_running():
		...
		Global.diffusion.set_prompt($RichTextLabel.text)
		
		Global.diffusion.set_param("width", 1024)
		Global.diffusion.set_param("height", 576)
		Global.diffusion.set_param("sample_method", 1)
